...
export const UserList = ({
    userList, handleCurrentEmployeeIndex, deleteData
}) => {
    const [loadingIndex, setLoadingIndex] = useState(0)
    return (
        <div className="">
            <Table
                tableHeading={["Name", "Email","Address", "Role", ""]}
                render={
                    ()=>(
                        <>
                        {userList.map((value, index) => (
                            <tr key={value._id}>
                                ...
                                <td className="first-letter:uppercase">{value.role}</td>
                                <td>
                                    <div className={`${loadingIndex === index ? "opacity-40 pointer-events-none" : ""}`}>
                                        <>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"
                                                ...
                                                onClick={() => { setLoadingIndex(index); handleCurrentEmployeeIndex(index); }}>
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"
                                                ...
                                                onClick={() => { setLoadingIndex(index); deleteData(value.id); }}
                                            >
